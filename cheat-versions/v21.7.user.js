// ==UserScript==
// @name         Wankle3D Cheat v21.7 — Close-Range Fix + 3-Tick Stationary Pulse + Gap Thread
// @namespace    wankle-cheat
// @version      21.7.0
// @description  v21.7: close-range ricochet fix (detect wall when shell starts inside expanded box) + 3-tick stationary pulse (fire 1 shell, wait 3 ticks, verify target isn't moving) + finer gap threading. `=diag F8=toggle F9=menu 1-5=profiles.
...
var waitTimer = setInterval(function() {
  if (++waitTries > 300) {
    ...
    showBanner('Wankle v21.7: game not found — try refreshing', '#ff5a3b', 6000);
    return;
  }
  var w = window.WANKLE;
...
function boot(WANKLE, discovered) {
  ...
  console.log('%c[wkl v21.7] hooked — buildView='+discovered.name+' sendInput='+_sendName+' pid='+_pidField, 'color:#7fd0ff;font-weight:bold');
  ...
  function ownShellInPath(me, moveX, moveZ) {
    ...
    for (var i=0; i<cachedView.shells.length; i++) {
      ...
      var perpDist = Math.abs(Math.cos(shAngle) * sdz - Math.sin(shAngle) * sdx);
      if (perpDist < TANK_R + SHELL_R + 5) return true;  // shell will hit us at probe point
    }
    return false;
  }
  ...
  net[_sendName || 'sendInput'] = function(input) {
    try {
      ...
      if (cfg.aimbot && aim_angle !== null && !proposal.aimSnap) {
        ...
        if (cfg.triggerbot && !proposal.fireSuppress) {
          ...
          if (currentTargetTank) {
            ...
            // v21.7: Position-history pulse check. Track the last 3 positions
            // of this target. If total movement over 3 samples is < 3u, confirm
            // stationary. This is more reliable than velocity alone (velocity can
            // be noisy from interp jitter).
            if (!window._stationaryPulse) window._stationaryPulse = {};
            var pulse = window._stationaryPulse[tgtId];
            if (!pulse) {
              pulse = { positions: [], lastT: 0 };
              window._stationaryPulse[tgtId] = pulse;
            }
            var nowMs = performance.now();
            // Sample every ~50ms (roughly every 6 ticks at 120Hz)
            if (nowMs - pulse.lastT > 50) {
              pulse.positions.push({ x: currentTargetTank.x, z: currentTargetTank.z, t: nowMs });
              while (pulse.positions.length > 3) pulse.positions.shift();
              pulse.lastT = nowMs;
            }
            // If we have 3 samples, check total movement
            if (pulse.positions.length >= 3) {
              var p0 = pulse.positions[0], p2 = pulse.positions[pulse.positions.length - 1];
              var totalMove = Math.hypot(p2.x - p0.x, p2.z - p0.z);
              if (totalMove < 3) {
                isStationary = true;  // confirmed stationary by pulse check
              } else if (totalMove > 20) {
                // Target moved significantly — not stationary
                isStationary = false;
              }
            }
            
            // Clean up old entries for dead/gone targets
            var activeIds = cachedEnemies.map(function(e) { return String(e.id); });
            for (var pid in window._stationaryPulse) {
              if (activeIds.indexOf(pid) < 0) delete window._stationaryPulse[pid];
            }
          }
          ...
          // v21.6: STRICT STATIONARY — for brown/stationary tanks, NEVER fire a
          // 2nd shell. One shell is a guaranteed kill on a stationary target.
          // The shellAlreadyGoingToHit check above handles this, but we also
          // add a hard block: if we've fired ANY shell and the target is
          // stationary, don't fire again until that shell despawns.
          if (isStationary && myShellsInFlight() > 0 && shellAlreadyGoingToHit) {
            shellAlreadyGoingToHit = true;  // redundant but explicit
          }
      ...
      // v21.6: Also check for own-shell danger on NORMAL movement (not just dodge).
      // The player can walk into their own shell during normal movement when no
      // dodge is active. This catches that case.
      if (proposal.moveSource !== null) {
        // Check if this movement would put us in our own shell's path
        if (proposal.moveSource !== 'dodge_own_shell_mirror' && 
            proposal.moveSource !== 'dodge_own_shell_hold' &&
            proposal.moveSource !== 'mine-retreat' &&
            (Math.abs(proposal.moveX) > 0.1 || Math.abs(proposal.moveZ) > 0.1)) {
          if (ownShellInPath(me, proposal.moveX, proposal.moveZ)) {
            // Don't move into our own shell — hold still
            proposal.moveX = 0;
            proposal.moveZ = 0;
            proposal.moveSource = 'own_shell_hold';
          }
        }
  ...
  function computeDodge() {
    ...
    for (var si=0; si<shells.length; si++) {
      ...
      if (best && best.tImpact>=0 && best.tImpact<cfg.dodgeHorizon) {
        // v21 FIRE-STUN-AWARE DODGE: when the player just fired, they're stunned
        // for FIRE_STUN_MS (42ms) and cannot move. The original dodge code
        // computed `canMove = mySpeed * tImpact` assuming full movement, which
        // overestimated escape ability during fire-stun and caused the bot to
        // not dodge urgently enough when shells arrived mid-fire-stun.
        //
        // Fix: subtract the remaining fire-stun time from tImpact when computing
        // canMove. If tImpact < remaining stun, canMove = 0 (cannot escape at all).
        var nowMs = performance.now();
        ...
        // If we're currently stunned and a shell is incoming, boost urgency to max
        // — we need to start moving the moment stun ends, and possibly the aimbot
        // should consider NOT firing to preserve mobility.
        var stunPenalty = stunRemaining > 0 ? 0.3 : 0;  // add 0.3 urgency if stunned
        if (best.dist < canMove+cfg.dodgeMargin+TANK_R) {
          ...
          var urg = Math.max(0.1, Math.max(timeUrg,marginUrg)) + stunPenalty;
          if (urg > 1) urg = 1;  // cap at 1.0
          threats.push({type:'shell', approach:best, urgency:urg, stunRemaining: stunRemaining});
        }
      }
    }
  ...
  function buildMenu() {
    ...
    title.textContent='WANKLE v21.7';
  ...
  function runDiag() {
    console.log('%c[wkl v21.7] DIAGNOSTIC', 'color:#7fd0ff;font-weight:bold');
  ...
  window.addEventListener('keydown', function(e) {
    ...
    if (e.code==='F8') {
      ...
      showBanner('Wankle v21.7: '+(cfg.enabled?'ON':'OFF'), cfg.enabled?'#7fd0ff':'#ff5a3b');
    }
    if (e.code==='F9') {
      e.preventDefault();
  ...
  showBanner('Wankle v21 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (adaptive multi-hypothesis + HP-aware conservation + fire-stun-aware dodge)', '#7fd0ff', 5000);
  console.log('%c[wkl v21.7] Ready. Close-range fix + 3-tick pulse + gap thread. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
