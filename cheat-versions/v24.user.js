// ==UserScript==
// @name         Wankle3D Cheat v24 — v23 Dodge + Re-enabled Conservation (50ms) + Aim Correction (15 samples, alpha 0.1, direct only) + Target Stickiness 1.2x
// @namespace    wankle-cheat
// @version      23.0.0
// @description  v24: fix 3 conflicts (pathHitsRect for bank shots, wire shellWillHitEnemy, shell angle source), path-crossing SKIP not penalty, ray accumulation fix, remove dead code (detectDeathCause, mine drilling, enemyHpTracker), remove aim correction offset from leadAim, per-respawn telemetry. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  var TARGET_STICKINESS_FACTOR = 1.1;  // v24: reduced from 1.2 — switch targets faster  // new target must be 1.5x better to switch
  ...
  function computeDodge() {
    ...
    // v24: POST-PROCESSING GUARD — ensure the final dodge direction NEVER points
    // toward an incoming shell. This is the critical fix for "dodgeTowardShell=True"
    // deaths. With 20+ shells, the vector sum of perpendiculars can align with the
    // direction TO one of the shells. This guard checks the final direction against
    // all nearby shells and rotates it 90° if it points toward any shell.
    var shells = cachedView.shells || [];
    var myId = getMyId();
    for (var gsi = 0; gsi < shells.length; gsi++) {
      var gs = shells[gsi];
      if (String(gs.o) === String(myId)) continue;
      var gsDist = Math.hypot(gs.x - me.x, gs.z - me.z);
      if (gsDist > 400 || gsDist < 1) continue;  // only check nearby shells
      // Direction from player to shell
      var toShellX = (gs.x - me.x) / gsDist, toShellZ = (gs.z - me.z) / gsDist;
      // Dot product: if dodge direction points toward shell (>0.2), rotate 90°
      var guardDot = dx * toShellX + dz * toShellZ;
      if (guardDot > 0.2) {
        // Rotate 90° perpendicular — try both rotations, pick the one that's safer
        var rot1X = -dz, rot1Z = dx;   // +90°
        var rot2X = dz, rot2Z = -dx;   // -90°
        // Pick the rotation that points AWAY from the most shells
        var score1 = 0, score2 = 0;
        for (var gsi2 = 0; gsi2 < shells.length; gsi2++) {
          var gs2 = shells[gsi2];
          if (String(gs2.o) === String(myId)) continue;
          var gs2Dist = Math.hypot(gs2.x - me.x, gs2.z - me.z);
          if (gs2Dist > 400 || gs2Dist < 1) continue;
          var toShell2X = (gs2.x - me.x) / gs2Dist, toShell2Z = (gs2.z - me.z) / gs2Dist;
          // Negative dot = moving AWAY from shell = good
          score1 += -(rot1X * toShell2X + rot1Z * toShell2Z);
          score2 += -(rot2X * toShell2X + rot2Z * toShell2Z);
        }
        if (score1 >= score2) { dx = rot1X; dz = rot1Z; }
        else { dx = rot2X; dz = rot2Z; }
  ...
  showBanner('Wankle v24 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (simultaneous dodge: vector always runs + cold-spot as bias, shot prediction wired, bank shot hitbox fix, ray accumulation fix, aim correction removed)', '#7fd0ff', 5000);
  console.log('%c[wkl v24] Ready. Simultaneous dodge: vector dodge ALWAYS runs (75%), cold-spot as BIAS (25%) with path-crossing check. Shot prediction wired in. pathHitsRect for bank shots. Ray accumulation fixed. Aim correction offset removed (keep complex prediction). Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
