// ==UserScript==
// @name         Wankle3D Cheat v22.5 — Per-Shell Own-Shell Dodge Fix
// @namespace    wankle-cheat
// @version      22.5.0
// @description  v22.5: v22.4 + per-shell vector dodge now includes OWN shells (fixes self-shell deaths on maps where cold-spot doesn't activate). Deep dive showed 2/4 Dungeon deaths were self-shell because the per-shell dodge skipped own shells. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  showBanner('Wankle v22.5 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (v22.4 + per-shell own-shell dodge fix — eliminates self-shell ricochet deaths)', '#7fd0ff', 5000);
  console.log('%c[wkl v22.5] Ready. v22.4 + per-shell vector dodge now includes own shells. Fixes self-shell deaths on Dungeon/RK Fight where cold-spot doesnt activate. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
