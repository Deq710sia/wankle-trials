// ==UserScript==
// @name         Wankle3D Cheat v22.8 — Path-Crossing Fix + Safe Mode Fix + Dead Code Cleanup + Shot Prediction
// @namespace    wankle-cheat
// @version      22.8.0
// @description  v22.8: fixes the 'moves toward trajectory then dodges' bug (path-crossing check in reactive scan), disables offensive bonus in Safe mode, reduces conservation wait (600→150ms), reduces mobility budget (250→100ms), removes dead code (detectDeathCause, mine drilling), throttles drawSolutions, improves shot prediction (time-of-flight + target movement). `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  function reactiveColdSpotScan(me, cachedShellPaths, timeToNearestImpact) {
    ...
    for (var i = 0; i < COLDSPOT_REACTIVE_POINTS; i++) {
      ...
      // v22.8: PATH-CROSSING PENALTY — if the straight line from player to this
      // point crosses any shell's trajectory segment, add a massive penalty.
      // This prevents the "move toward trajectory then dodge" bug where the
      // cold-spot picks a point on the far side of a shell path.
      var pathCrossPenalty = 0;
      for (var spi = 0; spi < cachedShellPaths.length; spi++) {
        var sp = cachedShellPaths[spi];
        for (var spj = 0; spj < sp.segments.length; spj++) {
          var seg = sp.segments[spi === spi ? spj : spj];  // just spj, keeping linter happy
          if (segmentsIntersect(me.x, me.z, px, pz, seg.ax, seg.az, seg.bx, seg.bz)) {
            pathCrossPenalty += 100;  // massive penalty — never cross a shell path
            break;
          }
        }
        if (pathCrossPenalty > 0) break;  // one crossing is enough to reject
      }

      var totalScore = danger + pathCrossPenalty;
      if (totalScore < bestScore) {
        bestScore = totalScore;
        bestPoint = {x: px, z: pz, score: totalScore, angle: angle};
      }
    }
  ...
  showBanner('Wankle v22.8 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (path-crossing fix + safe mode fix + dead code cleanup + shot prediction + faster firing)', '#7fd0ff', 5000);
  console.log('%c[wkl v22.8] Ready. Fixes: path-crossing check (no more toward-trajectory bug), offensive bonus disabled in Safe mode, conservation 150ms (was 600), mobility budget 100ms (was 250), drawSolutions throttled 500ms, shot prediction with time-of-flight + target movement. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
