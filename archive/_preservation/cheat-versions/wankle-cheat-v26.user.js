// ==UserScript==
// @name         Wankle3D Cheat v26 — v25 + Magnetize Root-Cause Fix + Tier-2 Prediction Engine + Randomized Safe-Direction + Dead-Code Cuts
// @namespace    wankle-cheat
// @version      26.0.0
// @description  v26: builds on v25 (path-segment guard, full 360° dodge, multi-angle burst, no stickiness) with: (1) magnetize root-cause fix (project out toward-shell component from input before blend, lower HIGH urgency 0.5->0.35); (2) Tier-2 prediction engine (adaptive EMA, acceleration tracking, inherited-velocity launch, per-target pattern memory); (3) randomized safe-direction selection (anti-human: when 2+ safe cells within 10% of best, pick random); (4) v25-opt dead-code cuts (mine drilling, aim-correction recording, hasRicochetShell, findPickup) + perf opts (per-frame stats cache, grid pointInTile). `=diag F8=toggle F9=menu 1-5=profiles.
...
var cfg = {
  ...
  // v22.8: reduced from 250 to 100 — was blocking too many shots, v19 has no budget and gets more kills
...
function boot(WANKLE, discovered) {
  ...
  function getAimCorrection(distance, targetSpeed, shotType) {
    ...
    if (!entry || (entry.hits + entry.misses) < 15) return {x: 0, z: 0};
  ...
  function recordShotResult(distance, targetSpeed, hit, errorX, errorZ, shotType) {
    ...
    if (hit) {
      ...
      var alpha = 0.1;  // v24: slower learning = more stable
  ...
  function isInConservationWait(targetId, now) {
    ...
    // v22.8: reduced from 600 to 150 per shell — was 1.8s after 3 shells, way too conservative
  ...
  // ═══════════════════════════════════════════════════════════════
  //  v26 PREDICTION ENGINE — Tier 1+2 rewrite + per-target pattern memory
  //  Drop-in replacement for v25's velTrack + updateVelTrack + getVel + leadAim.
  //  Changes:
  //    1. Adaptive EMA alpha (fast adapt on >30% speed change, 0.4 stable)
  //    2. Two-sample acceleration tracking (catches deceleration before reversal)
  //    3. Trend-weighted multi-hypothesis (reverse bias scales with flip recency)
  //    4. Inherited-velocity launch compensation (shell inherits myVel component)
  //    5. Velocity-variance hit-prob penalty (exposed via .uncertainty)
  //    6. Per-target pattern memory (tracks reversal period, predicts next)
  //  Interface UNCHANGED — all callers work. leadAim returns new .uncertainty (0..1).
  // ═══════════════════════════════════════════════════════════════

  // ── Velocity tracker (per-tank EMA velocity + acceleration + pattern memory) ──
  var velTrack = {};  // id -> {x, z, t, vx, vz, ax, az, prevVx, prevVz, samples, history, lastHistT, pattern}
    ...
    for (var i = 0; i < tanks.length; i++) {
      ...
      if (prev) {
        ...
        if (dt > 0.001 && dt < 0.5) {
          ...
          // (2) TWO-SAMPLE ACCELERATION — acc = (vel_now - vel_2_samples_ago) / (2 * dt)
          if (prev.prevVx != null) {
  ...
  // ── Lead-aim solver (v26: Tier 1+2 rewrite) ──
  ...
  function leadAim(me, tgt, shellSpeed) {
    ...
    // ── OWN MOVEMENT PREDICTION (unchanged) ──
    ...
    // Shell inherits player's velocity component along aim direction.
    // Iterate: start with shellSpeed, refine with computed aim direction.
    ...
    // Iterative refinement — uses pos + vel*t + ½·acc·t² (acceleration term)
    ...
    for (var iter = 0; iter < iterations; iter++) {
      ...
      if (aimLen > 1) {
        ...
        effectiveShellSpeed = Math.max(50, shellSpeed + dotMy * 0.5);  // 0.5 = partial inheritance (empirical, A/B)
      }
    ...
    // Reverse-bias scales with recency of last flip.
    var uncertainty = 0;
    ...
    // If stable reversal period (5+ reversals, CV < 15%), predict next reversal.
  ...
  // v25: REMOVED target stickiness. v21.7 has no stickiness and works great.
  // The bot switches targets instantly, enabling multi-angle burst fire.
  ...
  function pickTarget(me, enemies) {
    ...
    for (var i = 0; i < enemies.length; i++) {
      ...
      // v25: No stickiness — pure scoring, instant switching
  ...
  function searchBankShots(launchX, launchZ, tx, tz, hitR) {
    ...
    for (var rad = 0; rad < TAU; rad += stepRad) {
      ...
      // v24: use pathHitsRect for bank shots too — circular (35u) was 56% larger than actual hitbox (22.5u), wasting shells
    ...
    for (var rad2 = best.angle - stepRad; rad2 <= best.angle + stepRad; rad2 += fineStep) {
      ...
      // v24: use pathHitsRect for bank shots too
  ...
  // v24: IMPROVED shot prediction — traces the shell's FULL ricochet path
  // (not just current angle) to determine if it will hit the target.
  // This fixes the brown tank over-firing bug: after a bank shot bounces,
  // the angle changes but the shell is still heading at the target.
  // The old version only checked the current angle, missing post-bounce hits.
  ...
  net[_sendName || 'sendInput'] = function(input) {
    try {
      ...
      if (me.dead) {
        // v24: Clear visualization caches on death to prevent ray overpopulation
        // (user reported "screen gets overpopulated with rays after 10+ repeats")
        cachedSolutions = [];
        tracerCache = null;
        if (cfg.autoRespawn) {
      ...
      if (cfg.autoDodge && lastDodgeVec && !proposal.moveOverride) {
        ...
        // Path-segment guard (v25) treats the symptom at the dodge-vector level;
        // this fix addresses the cause at the input-blend level.
        // Fix: project out the input component that aligns with the shell's direction.
        ...
        // v26: HIGH threshold lowered 0.5 → 0.35 (narrower medium window for bug to manifest)
        if (dUrg > 0.35) {
          ...
          // MEDIUM urgency: strong blend toward dodge (using inputSafe — toward-shell projected out)
          var blend = dUrg * 1.6;
          ...
          // LOW urgency: gentle nudge (using inputSafe)
      ...
      if (cfg.aimbot && aim_angle !== null && !proposal.aimSnap) {
        ...
        if (cfg.triggerbot && !proposal.fireSuppress) {
          ...
          // v24: Wire in shellWillHitEnemy (time-of-flight + target movement prediction)
          // instead of the crude shellHeadingAtEnemy (perpendicular distance only).
          // This is the user's idea: "determine after a shot has been fired but before
          // it connects, how likely it is to connect, and based off of that shoot
          // another shot or dont shoot another shot."
          ...
          if (isStationary && currentTargetTank) {
            // v24: HARD RULE for stationary targets — if we have ANY shell in flight,
            // don't fire another. Brown tanks don't move, so 1 shell is always enough.
            // The shellWillHitEnemy check above may fail if the shell bounces (angle
            // changes post-bounce), but the shell is still going to hit. This hard
            // rule prevents ALL over-firing at stationary targets.
            if (myShellsInFlight() > 0) {
          ...
          // v25: REMOVED adaptive cooldown. v21.7 uses fixed 80ms cooldown which
          // produces multi-angle burst fire (12 shells/sec at different angles).
          // The adaptive cooldown was SLOWING DOWN firing, reducing the terrifying
          // burst-fire behavior. The brown tank hard rule + shellWillHitEnemy
          // handle conservation — no need for adaptive rate limiting.
  ...
  // v22.8: Segment-segment intersection check — does line (px1,pz1)→(px2,pz2)
  // cross segment (ax,az)→(bx,bz)? Used to prevent dodge from crossing shell paths.
  ...
  // v22.8: PATH-CROSSING CHECK — skip points that require crossing a shell's
  ...
  function reactiveColdSpotScan(me, cachedShellPaths, timeToNearestImpact) {
    ...
    // v26: collect ALL points within 10% of best score for randomized selection.
    // Anti-human: deterministic dodge is readable. Human watches 3 dodges, predicts 4th.
    // When 2+ safe cells are within 10% of best, pick RANDOM — breaks the pattern.
    var candidates = [];
    ...
    for (var i = 0; i < COLDSPOT_REACTIVE_POINTS; i++) {
      ...
      // v24: PATH-CROSSING SKIP — if the straight line from player to this
      // point crosses any shell's trajectory segment, SKIP this point entirely.
      ...
      if (pathCrosses) continue;  // SKIP — don't even consider this point
      if (danger < bestScore) {
        bestScore = danger;
    ...
    // v26: RANDOMIZED SAFE-DIRECTION SELECTION
    // If 2+ candidates are within 10% of best score, pick a random one.
    // This breaks deterministic dodge patterns that humans exploit.
    if (candidates.length >= 2 && bestScore < Infinity) {
      var threshold = bestScore * 1.10;  // within 10% of best
      var safeCandidates = [];
      for (var ci = 0; ci < candidates.length; ci++) {
        if (candidates[ci].score <= threshold) safeCandidates.push(candidates[ci]);
      }
      if (safeCandidates.length >= 2) {
        // Pick random from safe candidates — breaks pattern
  ...
  function strategicColdSpotGrid(me, cachedShellPaths, enemies, now) {
    ...
    for (var gx = -halfGrid; gx <= halfGrid; gx++) {
      for (var gz = -halfGrid; gz <= halfGrid; gz++) {
        ...
        // v22.8: only apply offensive bonus when aimbot is ON — in Safe mode (aimbot off),
        // pulling toward enemies is pure suicide (especially on Dodge Training with 72 bots).
        ...
        for (var ei = 0; ei < enemies.length; ei++) {
          ...
          // v22.8: DISABLED when aimbot is off (Safe mode)
  ...
  function computeDodge() {
    ...
    for (var si=0; si<shells.length; si++) {
      var s=shells[si];
      // v24: Don't include own shells in the main vector sum — they cancel
      // enemy shell perpendicular vectors. Instead, mark them as isOwn and
      // reduce their weight to 0.3x in the vector sum. This fixes the
      // "dies to single enemy shells" bug where own shell vectors canceled
      // the dodge direction.
    ...
    // v24: SIMULTANEOUS DODGE — vector dodge ALWAYS runs (immediate survival),
    // cold-spot runs as a BIAS (strategic pull toward safe zones). They NEVER
    // replace each other. This fixes the paralysis bug where cold-spot and
    // vector dodge conflicted, canceling each other out and locking the player
    // in place.
    //
    // Architecture:
    //   1. Vector dodge computes perpendicular direction (always safe by definition)
    //   2. Cold-spot computes direction toward safest area
    //   3. Check if cold-spot direction crosses any shell path within 300u
    //   4. If safe: blend 75% vector + 25% cold-spot (drift toward safe zone while dodging)
    //   5. If unsafe: use 100% vector (pure survival, no strategic pull)
    //   6. NEVER let cold-spot override vector — vector always dominates
    //
    // On Dodge Training: player dodges the 20-shell volley (vector) while
    // drifting toward the edges (cold-spot) where fewer shells converge.
    // On low-density maps: cold-spot finds genuinely safe positions and
    // gently pulls the player there between dodges.
    ...
    for (var ti=0; ti<threats.length; ti++) {
      ...
      if (th.type==='shell') {
        ...
        // v24: REMOVED v22.0 velocity-aware bias from per-shell vector. It caused
        // "dodging toward bullets" at high shell density — the reverse of the player's
        // velocity can point toward another shell. v21.7 doesn't have this and survives
        // every shot. The perpendicular + away weighting already handles direction.
      } else {
        var ddx=me.x-th.x, ddz=me.z-th.z, dd=Math.hypot(ddx,ddz);
        if (dd>0.1) { px=ddx/dd; pz=ddz/dd; }
      }
      // v24: Reduce own-shell weight to 0.3x — own shells can cancel enemy
      // shell perpendicular vectors if given full weight. 0.3x lets them
      // contribute to self-shell avoidance without overpowering enemy dodge.
    ...
    // v24: SIMULTANEOUS COLD-SPOT BIAS
    // After vector dodge has computed a safe direction (dx, dz), add a gentle
    // pull toward the cold-spot's safest area. This lets the player drift toward
    // safe zones (edges of Dodge Training) while still dodging shells.
    //
    // CRITICAL: Cold-spot is a BIAS (25%), not a replacement. Vector dodge (75%)
    // always dominates. The cold-spot direction is checked for path-crossing
    // BEFORE blending — if it crosses a shell path, it's skipped entirely.
    // v24: Cold-spot bias only at LOW shell density (2-5 shells). At 6+ shells,
    // the cold-spot can't find safe directions (all points cross paths) and its
    // 25% bias can pull the vector dodge TOWARD shells (confirmed by telemetry:
    // dodgeTowardShell=True on deaths 2 and 3). Pure vector dodge at high density.
    ...
    // v25: PATH-SEGMENT GUARD — replaces v24's dot-product guard.
    // The old guard checked if dodge direction points toward shell POSITION.
    // But shells MOVE along paths. The dodge can avoid the shell's current
    // position but still cross the shell's trajectory LINE — causing the
    // "magnetizing" bug where the player approaches the line then dodges.
    //
    // FIX: Check if the dodge direction (projected 100u forward) CROSSES
    // any shell's path segment. If it does, rotate in small steps (5°, 10°,
    // 15°... up to 90°) until finding a direction that doesn't cross any path.
    // This gives FULL 360° resolution (not 90° jumps) and fixes magnetizing.
    var shells = cachedView.shells || [];
    var myId = getMyId();
    ...
    var pathGuardRotation = 0;  // degrees of rotation applied (0 = no rotation needed)
    var pathGuardResolved = !crossesAny;  // true if final direction has 0 crossings
    if (crossesAny) {
      var bestRotX = dx, bestRotZ = dz;
      var bestRotCrosses = crossCount;
      var bestRotDeg = 0;
      // Try rotations from 5° to 175° in both directions
      for (var rotStep = 5; rotStep <= 175; rotStep += 5) {
        for (var rotDir = 0; rotDir < 2; rotDir++) {
          ...
          if (rCrosses < bestRotCrosses) {
            ...
            bestRotX = rX;
            bestRotZ = rZ;
            bestRotDeg = rotStep * (rotDir === 0 ? 1 : -1);
            if (rCrosses === 0) break;  // perfect — no crossings
          }
        }
        if (bestRotCrosses === 0) break;  // found a safe direction
      }
      dx = bestRotX;
      dz = bestRotZ;
      var dlen = Math.hypot(dx, dz);
      if (dlen > 0.01) { dx /= dlen; dz /= dlen; }
      pathGuardRotation = bestRotDeg;
      pathGuardResolved = (bestRotCrosses === 0);
    }

    // v25: telemetry expose for path-segment guard (so we can verify it engages)
    window._wklPathGuard = {
      shellsChecked: guardPaths.length,
      crossesAny: crossesAny,
      crossCount: crossCount,
      rotation: pathGuardRotation,
      resolved: pathGuardResolved,
    };
  ...
  function drawHUD(me, drawAim, drawBounces, drawDist, drawHitProb) {
    ...
    // v24: removed dead lastDeathCause HUD code (detectDeathCause was never called)
    ctx.fillStyle='#444';
  ...
  showBanner('Wankle v26 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (v25 base + magnetize root-cause fix + Tier-2 prediction + randomized safe-dir + dead-code cuts)', '#7fd0ff', 5000);
  console.log('%c[wkl v26] Ready. Builds on v25 (path-segment guard, 360° dodge, multi-angle burst, no stickiness). NEW: (1) magnetize root-cause fix (project out toward-shell input component, HIGH urgency 0.5→0.35); (2) Tier-2 prediction engine (adaptive EMA, acceleration, inherited-velocity, per-target pattern memory); (3) randomized safe-direction (anti-human: when 2+ safe cells within 10% of best, pick random); (4) v25-opt dead-code cuts. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
