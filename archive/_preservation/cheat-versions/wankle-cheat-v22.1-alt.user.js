...
function boot(WANKLE, discovered) {
  ...
  function refreshViewCache() {
    try {
      ...
      if (cachedMe) {
        ...
        // v22.1: Check pending shots for hit/miss evaluation
        checkPendingShots();
      }
  ...
  function getAimCorrection(distance, targetSpeed) {
    loadAimCorrections();
    var distBucket = isNaN(distance) ? 0 : Math.min(9, Math.floor(distance / 200));
  ...
  // v22.1 SHOT-BASED TRACKER (simpler, more reliable than shell-based)
  // Records each shot at fire time, then checks target HP after 2 seconds.
  // No shell ID tracking needed — just a queue of pending shots.
  ...
  // Called from refreshViewCache: check pending shots that are old enough to evaluate
  function checkPendingShots() {
    ...
    for (var i = 0; i < pendingShots.length; i++) {
      ...
      // Wait at least 1.5 seconds before evaluating (shell travel time)
      if (age < 500) {  // v22.1: reduced from 1500ms to 500ms for faster evaluation
        remaining.push(shot);
        continue;
      }
      // Find the target
      ...
      if (target) {
        if (target.dead) {
          ...
          // Miss — compute where target is NOW vs where we aimed
          errorX = target.x - shot.aimX;
          errorZ = target.z - shot.aimZ;
        }
      } else {
        // Target gone — assume hit (killed and despawned)
        hit = true;
      }
  ...
  net[_sendName || 'sendInput'] = function(input) {
    try {
      ...
      if (proposal.fireSuppress) { input.fire = false; }
        ...
        // v22.1: Record this shot for hit/miss analysis (always, even if aim_targetId is null)
        {
          ...
          if (_tgtForTrack) {
            ...
            var _tgtDist = Math.hypot(_tgtForTrack.x - me.x, _tgtForTrack.z - me.z);  // use actual target pos, not aim pos
