...
function boot(WANKLE, discovered) {
  ...
  function leadAim(me, tgt, shellSpeed) {
    ...
    // v23: AIM CORRECTION REMOVED — raw velocity prediction is more reliable.
  ...
  function searchBankShots(launchX, launchZ, tx, tz, hitR) {
    ...
    for (var rad = 0; rad < TAU; rad += stepRad) {
      ...
      // v23: use pathHitsRect for bank shots too — circular (35u) was 56% larger than actual hitbox (22.5u), wasting shells
    ...
    for (var rad2 = best.angle - stepRad; rad2 <= best.angle + stepRad; rad2 += fineStep) {
      ...
      // v23: use pathHitsRect for bank shots too
  ...
  // v22.8: IMPROVED shot prediction — instead of just checking if a shell is
  // "roughly heading at" the enemy, predict WHERE the enemy will be when the
  // shell arrives, and check if the shell's path hits that predicted position.
  ...
  //
  // Returns {willHit: bool, probability: 0-1}
  function shellWillHitEnemy(enemyX, enemyZ, enemyVx, enemyVz) {
    ...
    for (var i = 0; i < cachedView.shells.length; i++) {
      ...
      if (sdist < 1) return {willHit: true, probability: 1.0};

      // Is shell heading toward enemy?
      var sproj = (Math.cos(shAngle) * sdx + Math.sin(shAngle) * sdz) / sdist;
      if (sproj <= 0) continue;  // moving away

      // Time of flight: how long until shell reaches enemy's current position
      var timeToReach = sdist / spd;

      // v22.8: Predict where enemy will be when shell arrives
      var predEnemyX = enemyX + enemyVx * timeToReach;
      var predEnemyZ = enemyZ + enemyVz * timeToReach;

      // Perpendicular distance from PREDICTED enemy position to shell's path
      var pdx = predEnemyX - s.x, pdz = predEnemyZ - s.z;
      var pdist = Math.hypot(pdx, pdz);
      if (pdist < 1) return {willHit: true, probability: 1.0};

      var perpDist = Math.abs(Math.cos(shAngle) * pdz - Math.sin(shAngle) * pdx);

      // v22.8: probability based on perpendicular distance
      // If perpDist < HIT_HL (22.5u), the shell will hit the predicted position
      // If perpDist > HIT_HL * 2 (45u), very unlikely to hit
      var probability = Math.max(0, 1 - perpDist / (HIT_HL * 2));
      if (probability > 0.5) {
  ...
  net[_sendName || 'sendInput'] = function(input) {
    try {
      ...
      if (me.dead) {
        // v23: Clear visualization caches on death to prevent ray overpopulation
      ...
      if (cfg.aimbot && aim_angle !== null && !proposal.aimSnap) {
        ...
        if (cfg.triggerbot && !proposal.fireSuppress) {
          ...
          // v23: Wire in shellWillHitEnemy (time-of-flight + target movement prediction)
  ...
  function reactiveColdSpotScan(me, cachedShellPaths, timeToNearestImpact) {
    ...
    for (var i = 0; i < COLDSPOT_REACTIVE_POINTS; i++) {
      ...
      // v23: PATH-CROSSING SKIP — if the straight line from player to this
  ...
  function computeDodge() {
    ...
    for (var si=0; si<shells.length; si++) {
      var s=shells[si];
      // v23: Don't include own shells in the main vector sum — they cancel
    ...
    // v23: SIMULTANEOUS DODGE — vector dodge ALWAYS runs (immediate survival),
    ...
    for (var ti=0; ti<threats.length; ti++) {
      ...
      if (th.type==='shell') {
        ...
        // v23: REMOVED v22.0 velocity-aware bias from per-shell vector. It caused
      ...
      // v23: Reduce own-shell weight to 0.3x — own shells can cancel enemy
    ...
    // v23: SIMULTANEOUS COLD-SPOT BIAS
    ...
    // v23: Cold-spot bias only at LOW shell density (2-5 shells). At 6+ shells,
    ...
    // v23: POST-PROCESSING GUARD — ensure the final dodge direction NEVER points
  ...
  function drawHUD(me, drawAim, drawBounces, drawDist, drawHitProb) {
    ...
    // v23: removed dead lastDeathCause HUD code (detectDeathCause was never called)
    ctx.fillStyle='#444';
  ...
  showBanner('Wankle v23 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (simultaneous dodge: vector always runs + cold-spot as bias, shot prediction wired, bank shot hitbox fix, ray accumulation fix, aim correction removed)', '#7fd0ff', 5000);
  console.log('%c[wkl v23] Ready. Simultaneous dodge: vector dodge ALWAYS runs (75%), cold-spot as BIAS (25%) with path-crossing check. Shot prediction wired in. pathHitsRect for bank shots. Ray accumulation fixed. Aim correction offset removed (keep complex prediction). Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
