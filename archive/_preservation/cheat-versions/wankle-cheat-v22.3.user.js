// ==UserScript==
// @name         Wankle3D Cheat v22.3 — Cold-Spot Dodge System
// @namespace    wankle-cheat
// @version      22.3.0
// @description  v22.3: cold-spot dodge system (position-scoring not vector-averaging), predicted shell spawns, 8-dir sim fallback for unsolvable clumps, aim correction split by direct/bank shots, dodge telemetry exposed for bots. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  function getPredictedShells(me, enemies) {
    var predicted = [];
    var myVel = getVel(me.id);
    // Predict where player will be in 200ms (shell travel time at medium range)
    ...
    for (var i = 0; i < enemies.length; i++) {
      ...
      // If turret within 0.1 rad (~6°) of player, enemy is aiming at us
      if (angleDiff < 0.1) {
        var speed = 320;
    ...
    for (var gx = -halfGrid; gx <= halfGrid; gx++) {
      for (var gz = -halfGrid; gz <= halfGrid; gz++) {
        ...
        for (var ei = 0; ei < enemies.length; ei++) {
          var ed = Math.hypot(enemies[ei].x - cx, enemies[ei].z - cz);
  ...
  function coldSpotDodge(me, threats, cacheAge) {
    ...
    // Velocity bias: prefer moving in direction of current heading (avoids reverse penalty)
    var myVel = getVel(me.id);
  ...
  function computeDodge() {
    ...
    // Single-shell threats still use the per-shell vector dodge (works fine for 1 threat).
    // 2+ shells → vector averaging cancels out, so delegate to cold-spot position-scoring.
    var shellThreatCount = 0;
    for (var sti = 0; sti < threats.length; sti++) {
      if (threats[sti].type === 'shell') shellThreatCount++;
    }
    if (shellThreatCount >= 2) {
  ...
  showBanner('Wankle v22.3 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (cold-spot dodge system — position-scoring for 2+ shells)', '#7fd0ff', 5000);
  console.log('%c[wkl v22.3] Ready. Cold-spot dodge system. Reactive scan (32pt) + strategic grid (9x9) + dot-product guard + 8-dir sim fallback + predicted shells. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
