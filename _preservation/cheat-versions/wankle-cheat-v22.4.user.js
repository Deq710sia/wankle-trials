// ==UserScript==
// @name         Wankle3D Cheat v22.4 — Best-of-Synthesis (Cold-Spot + v19 8-dir + v21.7 stationary + v22.0 velocity + aim correction split)
// @namespace    wankle-cheat
// @version      22.4.0
// @description  v22.4: synthesis of best ideas from all versions. v22.3 cold-spot + v19's 8-dir sim as primary reactive + ALL shells as threats (fixes self-shell deaths) + v21.7 stationary enemy danger zones + v22.0 velocity-aware bias on cold-spot merge + aim correction split by direct/bank shots + cold-spot activates on 1+ missiles. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  // Uses shell path (with ricochets) to determine proximity
  function shellDangerAtPoint(px, pz, shells, maxBounces) {
    var totalDanger = 0;
    for (var i = 0; i < shells.length; i++) {
      var sh = shells[i];
      var distToShell = Math.hypot(sh.x - px, sh.z - pz);
      if (distToShell > COLDSPOT_MAX_SHELL_DIST) continue;

      // Trace shell path with ricochets (shellRadius=0 for dodge — exact centerline)
      var traced = traceRicochet(sh.x, sh.z, Math.cos(sh.angle) * sh.speed, Math.sin(sh.angle) * sh.speed, maxBounces, COLDSPOT_MAX_SHELL_DIST, 0);

      // Find closest approach to point (px, pz)
      ...
      var timeToImpact = minDist / sh.speed;
      var urgencyWeight = 1 / Math.max(0.1, timeToImpact + 0.1);
      var predictWeight = sh.isPredicted ? COLDSPOT_PREDICT_WEIGHT : 1.0;
  ...
  function reactiveColdSpotScan(me, shells, timeToNearestImpact) {
    var mySpeed = getMySpeed(me);
    ...
    for (var i = 0; i < COLDSPOT_REACTIVE_POINTS; i++) {
      ...
      var danger = shellDangerAtPoint(px, pz, shells, cfg.dodgeBounces);
      if (danger < bestScore) {
        bestScore = danger;
  ...
  function strategicColdSpotGrid(me, shells, enemies, now) {
    ...
    var currentDanger = shellDangerAtPoint(me.x, me.z, shells, cfg.dodgeBounces);
    ...
    for (var gx = -halfGrid; gx <= halfGrid; gx++) {
      for (var gz = -halfGrid; gz <= halfGrid; gz++) {
        ...
        var danger = shellDangerAtPoint(cx, cz, shells, cfg.dodgeBounces);
        ...
        // Enemy proximity penalty (don't camp in firing arcs)
        var enemyPenalty = 0;
        for (var ei = 0; ei < enemies.length; ei++) {
          var enemy = enemies[ei];
          var ed = Math.hypot(enemy.x - cx, enemy.z - cz);
          if (ed < 200) enemyPenalty += 5 * (1 - ed / 200);
          // v22.4: stationary enemy danger zone (from v21.7's stationary detection).
          // If enemy hasn't moved recently (use velTrack), treat its turret angle as a
          // permanent firing arc. Cells along that arc get extra penalty.
          if (typeof getVel === 'function' && ed < 600) {
            ...
            if (eSpeed < 10 && typeof enemy.turretAngle === 'number') {
              // Stationary enemy — check if cell is in its firing arc
              ...
              if (eToCellLen > 1) {
                ...
                if (arcDiff < 0.15) {
                  // Cell is in stationary enemy's firing arc — big penalty
        ...
        var totalScore = danger + pinningPenalty + enemyPenalty - pickupBonus;
  ...
  function coldSpotDodge(me, threats, cacheAge) {
    ...
    var reactive = reactiveColdSpotScan(me, allShells, timeToNearestImpact);
    var strategic = strategicColdSpotGrid(me, allShells, enemies, now);
  ...
  showBanner('Wankle v22.4 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (best-of-synthesis: cold-spot + 8-dir sim + own-shell dodge + stationary arcs + velocity bias + aim correction split)', '#7fd0ff', 5000);
  console.log('%c[wkl v22.4] Ready. Best-of-synthesis. v22.3 cold-spot + v19 8-dir sim as primary + ALL shells as threats (fixes self-shell) + v21.7 stationary enemy arcs + v22.0 velocity bias on cold-spot + aim correction split direct/bank. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
