// ==UserScript==
// @name         Wankle3D Cheat v25 — Path-Segment Guard + Multi-Angle Burst + No Stickiness + Full 360° Dodge
// @namespace    wankle-cheat
// @version      25.0.0
// @description  v25: fix magnetizing (path-segment guard instead of position dot-product), full 360° dodge (small rotation steps not 90° jumps), multi-angle burst firing (fixed 80ms cooldown, no adaptive), remove target stickiness entirely, brown tank hard rule retained. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  function leadAim(me, tgt, shellSpeed) {
    ...
    // v24: AIM CORRECTION REMOVED — raw velocity prediction is more reliable.
    // v19 has no correction and outperforms v22.8 in kill rate. The EMA-based
    // offset can be wrong, especially early when few samples exist. The complex
    // prediction (fire-stun accounting, iterative refinement) is KEPT — only
    // the correction OFFSET is removed.
  ...
  function reactiveColdSpotScan(me, cachedShellPaths, timeToNearestImpact) {
    ...
    for (var i = 0; i < COLDSPOT_REACTIVE_POINTS; i++) {
      ...
      // v22.8 used a penalty (+100) which didn't work because ALL 32 points
      // got the same penalty when 72 bots fire. SKIP means the point is
      // excluded from consideration — forces the scan to pick a point that
      // doesn't require crossing a shell path.
      // Distance filter: only check shells within 300u (avoids wasted checks on far shells)
      var pathCrosses = false;
      for (var spi = 0; spi < cachedShellPaths.length; spi++) {
        var sp = cachedShellPaths[spi];
        // Distance filter — skip shells that are too far to matter
  ...
  showBanner('Wankle v25 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (path-segment guard + full 360° dodge + multi-angle burst + no stickiness)', '#7fd0ff', 5000);
  console.log('%c[wkl v25] Ready. Path-segment guard (fixes magnetizing). Full 360° dodge (5° steps). Multi-angle burst (fixed 80ms). No stickiness. Brown tank hard rule. Conservation 50ms. Aim correction 15 samples. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
