// ==UserScript==
// @name         Wankle3D Cheat v22.6 — Radical Firing + v22.5 Dodge
// @namespace    wankle-cheat
// @version      22.6.0
// @description  v22.6: v22.5 dodge (100% Dungeon survival) + radical firing improvements: v19 aggression params, adaptive fire rate, target stickiness, burst fire, smarter bullet conservation, lower aim correction threshold. `=diag F8=toggle F9=menu 1-5=profiles.
...
function boot(WANKLE, discovered) {
  ...
  function getAimCorrection(distance, targetSpeed, shotType) {
    ...
    if (!entry || (entry.hits + entry.misses) < 2) return {x: 0, z: 0};
  ...
  showBanner('Wankle v22.6 ready  |  1-5=profiles  F8=toggle  F9=menu  `=diag  (v22.5 dodge + radical firing: adaptive fire rate + target stickiness + burst fire + v19 aggression)', '#7fd0ff', 5000);
  console.log('%c[wkl v22.6] Ready. v22.5 dodge (100% Dungeon survival) + radical firing improvements. Adaptive cooldown (40-150ms), target stickiness (1.5x), burst fire on close stationary, v19 aggression params, lower aim correction threshold. Profiles: 1=Rage 2=Legit 3=Safe 4=Ghost 5=ESP.', 'color:#7fd0ff;font-weight:bold');
}
